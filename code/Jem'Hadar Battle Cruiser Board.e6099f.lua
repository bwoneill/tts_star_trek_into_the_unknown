...
--Dominion JH Cruiser'
shipSize = Global.getTable("shipSize").large
...
local shipClass = getObjectFromGUID("023726")
local alertToken = getObjectFromGUID("e5f1bc")
local powerToken = getObjectFromGUID("f63e99")
local healthToken = getObjectFromGUID("8579bf")
local crewToken = getObjectFromGUID("fa8330")

function setUp()

    local shipBag = getObjectFromGUID("db53b7").clone()
    ...
    self.UI.setAttribute("beamBtn", "active", "true")
    ...
    myShip.setPosition({shipPos.x+1, shipPos.y, shipPos.z})
...
function scanCheck()
	...
	if object.getStateId() == 2 or object.getStateId() == 3 then _range = 2 end
	if object.getStateId() == 1 or object.getStateId() == 4 then _range = 1 end
	if object.getStateId() == 5 or object.getStateId() == 6 then _range = 0 end
